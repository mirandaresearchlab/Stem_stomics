...
This is a minimal copy of scripts/stem_train.py with the following changes:
- swap DDPM loss for FlowMatching (continuous t in [0,1])
- model head outputs a single velocity channel (learn_sigma=False)
- optional [flow] config section for time_scale/time_eps
"""
...
class Trainer:
    def __init__(
        ...
        self.fm = create_flow_matching(time_eps=flow_time_eps, time_scale=flow_time_scale)
        ...
        self.args.logger.info(
            f"Rank {rank} - Initializing FM Trainer... DiT Parameters: {sum(p.numel() for p in model.parameters()):,}"
        )
    ...
    def _run_batch(self, x, t, modelkwargs):
        loss_dict = self.fm.training_losses(self.model, x, model_kwargs=modelkwargs, t=t)
    ...
    def _run_epoch(self, epoch):
        ...
        for x, y in self.train_data:
            ...
            t = torch.rand(x.size(0), device=x.device)  # continuous time in [0, 1]
...
def load_train_objs(args):
    ...
    model = Stem_models[args.model](
        ...
        learn_sigma=False,  # single velocity channel for flow matching
    )
...
def main(world_size: int, available_gpus: list, cfg: TrainingConfig):
    ...
    # resolve flow hyperparameters (fallback to defaults if not provided)
    # flow settings come from the [flow] section; default to native [0,1] times
    flow_time_scale = getattr(cfg, "time_scale", 1.0)
    flow_time_eps = getattr(cfg, "time_eps", 1e-3)
    cfg.logger.info(f"Flow settings — time_scale: {flow_time_scale}, time_eps: {flow_time_eps}")
...
def parse_args() -> Path:
    parser = argparse.ArgumentParser(description="Train DiT model with Flow Matching using a TOML config")
    parser.add_argument(
        "-c",
        "--config",
        type=Path,
...
def _cli_entrypoint():
    ...
    cfg: TrainingConfig = load_toml_config(cfg_path, TrainingConfig, ["model", "data", "training", "flow"])
